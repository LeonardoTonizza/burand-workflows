...
let WorkflowExecutionService = class WorkflowExecutionService {
    ...
    async create(templateWorkflowId, payload) {
        const template = await this.workflowTemplateRepository.getById(templateWorkflowId);
        const templateVersion = await this.workflowTemplateVersionRepository.getByVersion(templateWorkflowId, template.activeVersion);
    ...
    async generateGoogleLoggingURL(projectId, workflowExecutionId) {
        ...
        const link = generateGoogleLoggingURL(projectId, worker.steps.map(s => s.trace).filter((s) => !!s));
        return link;
    }
};
